...
public class VerisDevice extends SensorDevice implements DataRetrieval, Parcelable {
	// debugging
	...
	private class ModbusRTUClient implements Runnable {
		...
		public ModbusRTUClient(String ip, int p, int m, int r, 
				int q) throws UnknownHostException {	
			...
			tempBuf[1] = MODBUS_FUNC_READ_REG;
	...
	public void subscribeToSensor(int id) {
		...
		try {
			threadPool.execute(new ModbusRTUClient(ip_address, 
					port, modbus_addr, modbus_reg_addr, modbus_reg_qty));
